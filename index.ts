...
// ----------------------
// 🟢 Profile upsert route (partial-friendly, camelCase I/O)
// ----------------------
app.post('/api/profile', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ודאי שיש UNIQUE KEY על profiles.user_id
    await db.query(
...
// ----------------------
// 🟢 Get profile route (returns camelCase; safe defaults)
// ----------------------
...
// ----------------------
// 🟢 Helper: run Python script via PythonShell
// ----------------------
...
app.post('/api/ml-products', async (req: Request, res: Response) => {
  try {
    ...
    const products: any[] = results[0]; // פייתון מחזיר מערך מוצרים

    // שמירת הבחירות + המוצרים ב־MongoDB (אם קיים)
    if (mongoDB) {
